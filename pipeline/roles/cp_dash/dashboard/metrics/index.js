	...
	var latency = setInterval( function () 
	{
		for( var server of servers )
		{
			if( server.url )
			{
				...
				{
					// TASK 2
                                        // console.log("Task 2:",captureServer)
                                        // console.log("Latency: ",res.timings.end - res.timings.start)
...
function updateHealth(server)
{
	...
	// Update score calculation.
        ...
        if (server.latency > 15)
        {
            score = score - 1
        }
        else if (server.latency > 10){
            score = score - 0.5
        }
        if ((server.memoryLoad).split(" %",1) > 25)
        {
            score = score - 1
        }
        if ((server.cpu).split(" %",1) > 1)
        {
            score = score - 1
        }
